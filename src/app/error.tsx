...
/**
 * Error page with animated illustration and recovery options
 */
...
/**
 * Animated error illustration with robot graphic
 */
    ...
    >
      ...
      >
        <div className="relative overflow-hidden rounded-3xl border border-border/50 bg-card/50 p-8 shadow-2xl backdrop-blur-xl md:p-12">
          ...
          >
            <Image
              src="/Error-Lamp-Robot.svg"
              alt="Error Lamp Robot"
              width={400}
              height={400}
              priority
              className="h-auto w-[280px] drop-shadow-2xl transition-transform duration-700 group-hover:scale-110 md:w-[350px] lg:w-[400px]"
            />
            {/* ✨ Dynamic "Short-Circuit" Flicker Effect */}
            <motion.div
              className="pointer-events-none absolute inset-0 z-20 rounded-lg bg-destructive/10"
              animate={{ opacity: [0, 0.1, 0, 0.05, 0, 0.15] }}
        ...
        <motion.div
          className="absolute left-1/2 top-1/2 -z-10 h-80 w-80 -translate-x-1/2 -translate-y-1/2 rounded-full bg-destructive/20 blur-3xl"
...
/**
 * Main content with title, description, and actions
 */
function ContentSection({ reset }: { reset: () => void }) {
  return (
    <motion.div
      variants={animations.staggered}
    ...
    >
      ...
      <Card className="bg-card/50 backdrop-blur-lg">
        ...
        <CardFooter className="p-8 pt-0 md:p-10">
          <HelpText />
        </CardFooter>
      </Card>
    </motion.div>
...
function ErrorBadge() {
  return (
    ...
    >
      <motion.div
        className="h-2.5 w-2.5 rounded-full bg-destructive shadow-lg"
...
function Description() {
  return (
    ...
    >
      An unexpected error occurred. Please try to reload the page or return to
      the homepage.
    </motion.p>
  );
}
