...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  const normalizedImages = pool.map((image) => {
    if (typeof image === "string") {
      return { src: image, alt: "", name: "", role: "", institution: "" };
    }
    return {
      src: image.src || "",
      alt: image.alt || "",
  ...
  return coords.map((c, i) => ({
    ...
    institution: usedImages[i].institution,
  }));
}
...
export default function DomeGallery({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const rawRole = parent.dataset.role || "";
    const rawInstitution = parent.dataset.institution || "";
    ...
    // Add text overlay if name exists
    if (rawName) {
      ...
      nameEl.style.cssText = `
        ...
        line-height: 1.3;
      `;
      textOverlay.appendChild(nameEl);
      ...
      if (rawRole) {
        ...
        roleEl.style.cssText = `
          ...
          line-height: 1.4;
        `;
        textOverlay.appendChild(roleEl);
      }

      if (rawInstitution) {
        const institutionEl = document.createElement("div");
        institutionEl.textContent = rawInstitution;
        institutionEl.style.cssText = `
          font-size: 14px;
          opacity: 0.85;
          line-height: 1.4;
        `;
        textOverlay.appendChild(institutionEl);
      }
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                <div
                  ...
                  data-name={it.name || ""}
                  data-role={it.role || ""}
                  data-institution={it.institution || ""}
                  data-offset-x={it.x}
                  data-offset-y={it.y}
